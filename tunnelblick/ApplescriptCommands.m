...
- (id)performDefaultImplementation
{
    ...
    if (  connection  ) {
        if (  ! [connection isDisconnected]  ) {
            [connection addToLog: @"AxionVPN: Disconnecting; AppleScript 'disconnect' invoked"];
...
- (id)performDefaultImplementation
{
    ...
    while (  (connection = [connEnum nextObject])  ) {
        if (  ! [connection isDisconnected]  ) {
            [connection addToLog: @"AxionVPN: Disconnecting; AppleScript 'disconnect all' invoked"];
            [connection startDisconnectingUserKnows: [NSNumber numberWithBool: YES]];
            nDisconnecting++;
        }
    }
    ...
    while (  (connection = [connEnum nextObject])  ) {
        if (  ! [connection isDisconnected]  ) {
            ...
            if (  ! (   [gTbDefaults boolForKey: autoConnectkey]
                ...
                [connection addToLog:@"AxionVPN: Disconnecting; AppleScript 'disconnect all except when computer starts' invoked"];
