...
TBSYNTHESIZE_OBJECT_GET(retain, NSButton *, saveCredsInKeychainCheckbox)
...
-(void) awakeFromNib
{
    [[self window] setTitle: NSLocalizedString(@"AxionVPN: Login Required", @"Window title")];
    ...
    [saveCredsInKeychainCheckbox setTitle: NSLocalizedString(@"Save in Keychain", @"Checkbox name")];
...
-(void) redisplay
{
    ...
    [[self saveCredsInKeychainCheckbox] setState: NSOnState]; //   (   haveSavedUsername
                                                 //       && enableSaveCheckbox ? NSOnState : NSOnState)];  // Defaults to "checked" if have already saved username and not "connect when system starts"
    [[self saveCredsInKeychainCheckbox] setEnabled: enableSaveCheckbox];
