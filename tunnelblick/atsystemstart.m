...
int main(int argc, char* argv[])
{
    ...
    if (   ([execComponents count] != 5)
        ...
        NSLog(@"AxionVPN must be in /Applications (bundlePath = %@)", resourcesPath);
        errorExit();
    }
#endif
...
void setStart(NSString * plistPath, NSString * daemonDescription, NSString * daemonLabel, int argc, char* argv[])
{
    ...
    if (  [[NSFileManager defaultManager] tbPathContentOfSymbolicLinkAtPath: plistPath] != nil  ) {
        NSLog(@"AxionVPN atsystemstart: Symbolic link not allowed at %@", plistPath);
        errorExit();
    }
    
    if (  ! [plistDict writeToFile: plistPath atomically: YES]  ) {
        NSLog(@"AxionVPN atsystemstart: Unable to write plist file %@", plistPath);
        errorExit();
    }
}
