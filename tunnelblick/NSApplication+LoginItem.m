...
-(void) deleteOurLoginItemLeopardOrNewer {
    ...
    NSURL * ourURL = [NSURL fileURLWithPath: @"/Applications/AxionVPN.app/"];
...
-(void) deleteOurLoginItemTiger {
    ...
    for (  ix = 0; ix < [loginItems count]; ix++  ) {
        ...
        if (  [[item objectForKey: @"Path"] isEqualToString: @"/Applications/AxionVPN.app"]  ) {
...
-(void) setupNewAutoLaunchOnLogin {
    ...
    NSString * ourPlistPath = @"/Applications/AxionVPN.app/Contents/Resources/net.tunnelblick.tunnel" @"blick.LaunchAtLogin.plist";
