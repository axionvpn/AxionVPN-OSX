...
ARG_TB_PATH="/Applications/AxionVPN.app"
...
while [ {$#} ] ; do
    if [ "$1" = "-6" ] ; then                       # -6 = ARG_ENABLE_IPV6_ON_TAP (for TAP connections only)
        ...
        ARG_TB_PATH="${1:2}"				        # -t path of AxionVPN.app
        shift
...
#
# If AxionVPN.app is located in /Applications, we load the launchd .plist directly from within the .app.
#
# If AxionVPN.app is not located in /Applications (i.e., we are debugging), we create a modified version of the launchd .plist and use
...
if [ "${ARG_TB_PATH}" = "/Applications/AxionVPN.app" ] ; then
