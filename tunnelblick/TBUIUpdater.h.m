...
-(void) timerTickHandler: (NSTimer *) theTimer {
    if (  timer == theTimer  ) {
        ...
        // Update the Settings window and the Advanced window if one of the connections to update is selected
        ...
        if (   [connectionsToUpdate containsObject: connection]  ) {
			...
			if (   vpnDetailsWindow
				...
				if (  vpnDetailsWindow  ) {
					...
					TBLog(@"DB-UU", @"TBUIUpdater timerTickHandler: set title of Settings window to '%@'", statusMsg);
				}
				if (   advancedWindow
					&& showingAdvancedWindow  ) {
