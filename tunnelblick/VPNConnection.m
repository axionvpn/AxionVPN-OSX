...
extern NSString *memPassword;
extern NSString *memUserName;
...
// Reinitializes a connection -- as if we Quit AxionVPN and then relaunched
-(void) reInitialize
{
...
-(void) didHookup
{
    ...
    [self addToLog: @"AxionVPN: Established communication with OpenVPN"];
}
...
- (void) dealloc
{
	...
	[statistics.lastSet               release]; statistics.lastSet               = nil;
    [remoteIP                         release]; remoteIP                         = nil;
    [acctType                         release]; acctType                         = nil;
    ...
    if (   (! gShuttingDownTunnelblick)
        ...
        if (  ! showingStatusWindow  ) {
            ...
            if (  ! [statusPref isEqualToString: @"neverShow"]  ) {
                ...
                [statusScreen setStatus: lastState forName: displayName connectedSince: [self timeString] to:remoteIP oftype:acctType];
                [statusScreen fadeIn];
...
-(void) startCheckingIPAddressAfterConnected
{
    ...
    if (   [self ipAddressBeforeConnect]
        ...
        [self addToLog: [NSString stringWithFormat: @"AxionVPN: Could not determine this computer's apparent public IP address before the connection was completed"]];
...
-(BOOL) checkForChangedIPAddress: (NSString *) beforeConnect andIPAddress: (NSString *) afterConnect
{
	if (  [beforeConnect isEqualToString: afterConnect]  ) {
		[self addToLog: [NSString stringWithFormat: @"AxionVPN: This computer's apparent public IP address (%@) was unchanged after the connection was made", beforeConnect]];
		[self ipInfoNoChangeDialogBefore: beforeConnect];
		return FALSE;
	} else {
		[self addToLog: [NSString stringWithFormat: @"AxionVPN: This computer's apparent public IP address changed from %@ before connection to %@ after connection", beforeConnect, afterConnect]];
		return TRUE;
	}
}	
...
-(void) checkIPAddressAfterConnectedThread: (NSString *) threadID
{
    ...
    if (  ! ipInfo  ) {
        ...
        [self performSelectorOnMainThread: @selector(checkIPAddressErrorResultLogMessage:)
                               withObject: @"AxionVPN: An error occured fetching IP address information after connecting"
                            waitUntilDone: NO];
        [((MenuController *)[NSApp delegate]) haveFinishedIPCheckThread: threadID];
        [threadPool drain];
        return;
    ...
    if (  [self ipCheckLastHostWasIPAddress]  ) {
        ...
        [self performSelectorOnMainThread: @selector(checkIPAddressBadResultLogMessage:)
                               withObject: [NSString stringWithFormat: @"AxionVPN: After %.1f seconds, gave up trying to fetch IP address information using the ipInfo host's IP address after connecting.", timeoutToUse]
		                    waitUntilDone: NO];
        [((MenuController *)[NSApp delegate]) haveFinishedIPCheckThread: threadID];
        [threadPool drain];
        return;
    ...
    [self performSelectorOnMainThread: @selector(addToLog:)
                           withObject: [NSString stringWithFormat: @"AxionVPN: After %.1f seconds, gave up trying to fetch IP address information using the ipInfo host's name after connecting.", (double) timeoutToUse]
                        waitUntilDone: NO];
    ...
    if (  ! ipInfo  ) {
        NSLog(@"An error occured fetching IP address information after connecting");
        [self performSelectorOnMainThread: @selector(checkIPAddressErrorResultLogMessage:)
                               withObject: @"AxionVPN: An error occured fetching IP address information using the ipInfo host's IP address after connecting"
                            waitUntilDone: NO];
        [((MenuController *)[NSApp delegate]) haveFinishedIPCheckThread: threadID];
        [threadPool drain];
        return;
    ...
    [self performSelectorOnMainThread: @selector(checkIPAddressNoDNSLogMessage:)
                           withObject: [NSString stringWithFormat: @"AxionVPN: fetched IP address information using the ipInfo host's IP address after connecting."]
                        waitUntilDone: NO];
    [((MenuController *)[NSApp delegate]) haveFinishedIPCheckThread: threadID];
    [threadPool drain];
}
...
- (void) connect: (id) sender userKnows: (BOOL) userKnows
{
    ...
    if (  ! [gTbDefaults boolForKey:@"skipWarningAboutSimultaneousConnections"]  ) {
        ...
        //AXION Prevent multiple connections here
        if (  numConnections != 1  ) {
            
            NSLog(@"[connect] Closing old connection %@");
    ...
    NSString * logText = [NSString stringWithFormat:@"AxionVPN: Attempting connection with %@%@; Set nameserver = %@%@",
                          [self displayName],
    ...
    [self addToLog: [NSString stringWithFormat: @"AxionVPN: openvpnstart %@",
    ...
    if (  status != EXIT_SUCCESS  ) {
        ...
        if (  status == OPENVPNSTART_RETURN_SYNTAX_ERROR  ) {
            openvpnstartOutput = @"Internal AxionVPN error: openvpnstart syntax error";
        } else {
            openvpnstartOutput = stringForLog(errOut, @"AxionVPN: openvpnstart log:\n");
        }
        
        [self addToLog: [NSString stringWithFormat: @"AxionVPN:\n\n"
        ...
        if (  userKnows  ) {
            TBShowAlertWindow(NSLocalizedString(@"Warning!", @"Window title"),
                              [NSString stringWithFormat:
                               NSLocalizedString(@"AxionVPN was unable to start OpenVPN to connect %@. For details, see the log in the Settings window", @"Window text"),
                               [self localizedName]]);
            requestedState = oldRequestedState;
        }
    } else {
...
- (void) connectNoChecks: (id) sender userKnows: (BOOL) userKnows
{
	(void) sender;
    
    NSLog(@"Connecting to conf without checking");
    ...
    NSLog(@"Passed areConnecting Mutex");
    ...
    if (  ! [gTbDefaults boolForKey:@"skipWarningAboutSimultaneousConnections"]  ) {
        ...
        //AXION Prevent multipel connections here
        if (  numConnections != 1  ) {
            int button = TBRunAlertPanelExtended(NSLocalizedString(@"Do you wish to connect?", @"Window title"),
                                                 [NSString stringWithFormat:NSLocalizedString(@"Multiple simultaneous connections would be created (%d with 'Set nameserver', %d without 'Set nameserver').", @"Window text"), numConnectionsWithModifyNameserver, (numConnections-numConnectionsWithModifyNameserver) ],
                                                 NSLocalizedString(@"Connect", @"Button"),  // Default button
    ...
    NSLog(@"AXION Checkpoint 0");
    [self startCheckingIPAddressBeforeConnected];
    NSLog(@"AXION Checkpoint 0.1");
    ...
    NSLog(@"AXION Checkpoint 0.2");
    
    [self setArgumentsUsedToStartOpenvpnstart: [self argumentsForOpenvpnstartForNow: YES]];
    NSLog(@"AXION Checkpoint 0.3");

    
    connectedUseScripts    = (unsigned)[[argumentsUsedToStartOpenvpnstart objectAtIndex: OPENVPNSTART_ARG_USE_SCRIPTS_IX] intValue];
    NSLog(@"AXION Checkpoint 0.4");

    [self setConnectedCfgLocCodeString: [argumentsUsedToStartOpenvpnstart objectAtIndex: OPENVPNSTART_ARG_CFG_LOC_CODE_IX]];
    
    NSLog(@"AXION Checkpoint 0.5");
    if (  argumentsUsedToStartOpenvpnstart == nil  ) {
        if (  userKnows  ) {
            requestedState = oldRequestedState; // User cancelled
        }
    ...
    NSString * errOut;
    NSLog(@"AXION Checkpoint 3");

    BOOL isDeployedConfiguration = [[argumentsUsedToStartOpenvpnstart objectAtIndex: 5] isEqualToString:@"2"];

    OSStatus status = runOpenvpnstart(argumentsUsedToStartOpenvpnstart, nil, &errOut);
	
    NSLog(@"AXION Checkpoint 4");
    NSString * openvpnstartOutput;
    if (  status != EXIT_SUCCESS  ) {
		
         NSLog(@"AXION OpenVPN Start Failed");
...
-(BOOL) shadowIsIdenticalMakeItSo: (BOOL) makeItSo
{
	...
	if (  ! [[name pathExtension] isEqualToString: @"tblk"]) {
		NSLog(@"Internal AxionVPN error: '%@' is not a .tblk", name);
		return NO;
	}
	...
	switch (  status  ) {
		...
		case OPENVPNSTART_COMPARE_CONFIG_DIFFERENT:
			...
			AuthorizationRef authRef = [NSApplication getAuthorizationRef:
										NSLocalizedString(@"AxionVPN needs to create or update a secure (shadow) copy of the configuration file.", @"Window text")];
			if ( authRef == nil ) {
		...
		default:
			NSLog(@"Internal AxionVPN error: unknown status %ld from compareShadowCopy(%@)", (long) status, [self displayName]);
            TBShowAlertWindow(NSLocalizedString(@"Warning", @"Window title"),
							  [NSString stringWithFormat: NSLocalizedString(@"An error (status %ld) ocurred while trying to"
																			...
																			@"Please quit and relaunch AxionVPN. If the problem persists, please"
																			@" reinstall AxionVPN.", @"Window text"),
...
-(NSString *) setTunOrTapAndHasScrambleAndHasAuthUserPass {
    ...
    NSLog(@"[setTunOrTapAndHAsScableAndHAsAuthUserPass] Called");
    tunOrTap = [[NSString stringWithFormat:@"tunOrTap"] copy];
    return tunOrTap;
...
-(NSArray *) argumentsForOpenvpnstartForNow: (BOOL) forNow
{
    NSLog(@"[argsForOpenVPNStartForNow] Called");
    NSString * cfgPath = [self configPath];

    NSLog(@"[argsForOpenVPNStartForNow] [self configPath: %@",[self configPath]);
    ...
    NSLog(@"about to build array");
    NSLog(@"cfgPath: %@",cfgPath);
    NSLog(@"LastPartofPath(cfgPath): %@",lastPartOfPath(cfgPath));
    ...
    NSArray * args = [NSArray arrayWithObjects:
                     ...
                     //AXION [lastPartOfPath(cfgPath) copy] autorelease]
                     //AXION [[cfgPath copy] autorelease],
...
- (IBAction) toggle: (id) sender
{
	if (![self isDisconnected]) {
        [self addToLog: @"AxionVPN: Disconnecting; 'Disconnect' (toggle) menu command invoked"];
        ...
        [self addToLog: @"AxionVPN: Disconnecting; 'Connect' (toggle) menu command invoked"];
...
- (BOOL) startDisconnectingUserKnows: (NSNumber *) userKnows {
    ...
    if (   ALLOW_OPENVPNSTART_KILL
		&& (thePid > 0)  ) {
		[self addToLog: @"AxionVPN: Disconnecting using 'kill'"];
		...
		[self addToLog: @"AxionVPN: Disconnecting using 'killall'"];
		...
		[self addToLog: @"AxionVPN: Disconnecting using management interface"];
    ...
    //AXION - FAIL - Trying to delete here causes serious fail
...
exit:
    //AXION - trying to delete here does nothing
    //[((MenuController *)[NSApp delegate]) deleteExistingConfig:@"axion" ];
...
-(void) hasDisconnected {
    ...
    NSLog(@"[hasDisconnected] Called with %@",[self displayName]);
    ...
    //Axion - Delete the configuration
    [((MenuController *)[NSApp delegate]) deleteExistingConfig:[self displayName] ];
    ...
    if (   ( ! [requestedState isEqualToString: @"EXITING"])
        ...
        NSTimeInterval interval = (NSTimeInterval)[gTbDefaults unsignedIntForKey: @"timeoutForOpenvpnToTerminateAfterDisconnectBeforeAssumingItIsReconnecting"
                                                                         default: 5
                                                                             min: 0
                                                                             max: 60];
        [self addToLog: [NSString stringWithFormat: @"AxionVPN: Unexpected disconnection. requestedState = %@; waiting up to %.1f seconds for OpenVPN process %@ to terminate...",
        ...
        [self addToLog: @"AxionVPN: Expected disconnection occurred."];
	}
}
...
-(void) processState: (NSString *) newState dated: (NSString *) dateTime
{
    if ([newState isEqualToString: @"EXITING"]) {
        ...
        if ([newState isEqualToString: @"CONNECTED"]) {
            ...
            NSString *usernameLocal = memUserName;
            NSString *passwordLocal = memPassword;
            NSDictionary *status = [AxionAPI GetVPNStatus:usernameLocal andPass:passwordLocal];
            NSLog(@"status: %@",status);
            
            
            //AXion - copy
            
            //[: [[tempVPNConnectionDictionary copy] autorelease]];
            remoteIP = [status[@"ip_address"] copy];
            acctType = [status[@"acc_type"] copy];
            
            
            //Check for basic account
            if([acctType containsString:@"asic"]){
                NSLog((@"Basic Account Found!"));
                
                NSAlert *alert = [[NSAlert alloc] init];
                [alert addButtonWithTitle:@"Upgrade"];
                [alert addButtonWithTitle:@"Cancel"];
                // [alert ad]
                [alert setMessageText:@"You are using AxionVPN Basic"];
                [alert setInformativeText:@"For full unthrottled speed please upgrade to AxionVPN Pro"];
        ...
        if ([command isEqualToString: @"STATE"]) {
            ...
            if (   [line rangeOfString: @"Failed"].length
                ...
                if (alertVal == NSAlertDefaultReturn) {
                    ...
                    [self addToLog: @"AxionVPN: Disconnecting; user cancelled authorization or there was an error obtaining authorization"];
...
-(void) provideCredentials: (NSString *) parameterString line: (NSString *) line
{
    ...
    if (pwrange_need.length && pwrange_password.length) {
        ...
        if (  [myAuthAgent authenticationWasFromKeychain]  ) {
            [self addToLog: @"AxionVPN: Obtained VPN passphrase from the Keychain"];
        }
        ...
        if(  myPassphrase != nil  ){
            ...
            [self addToLog: @"AxionVPN: Disconnecting; user cancelled authorization"];
            [self startDisconnectingUserKnows: [NSNumber numberWithBool: YES]];      // (User requested it by cancelling)
        }
        ...
        [myAuthAgent performAuthentication];
        if (  [myAuthAgent authenticationWasFromKeychain]  ) {
            [self addToLog: @"AxionVPN: Obtained VPN username and password from the Keychain"];
        }
...
-(void) updateStatisticsDisplay {
	...
	if (  statusScreen) {
		[statusScreen setStatus: [self state] forName: [self displayName] connectedSince: [self timeString] to:remoteIP oftype:acctType];
	...
	// Be sure to call this in main thread only
{
    if (  [newState isEqualToString: @"EXITING"]  ) {
        ...
        if (  (connectedUseScripts & OPENVPNSTART_USE_SCRIPTS_RUN_SCRIPTS) != 0  ) {        //
            if (  [gFileMgr fileExistsAtPath: DOWN_SCRIPT_NEEDS_TO_BE_RUN_PATH]  ) {
                ...
                [self addToLog: [NSString stringWithFormat: @"AxionVPN: OpenVPN appears to have crashed -- the OpenVPN process has terminated without running a 'down' script, even though it ran an 'up' script. Tunnelblick will run the 'down' script #%@ to attempt to clean up network settings.", scriptNumberString]];
                if (  [scriptNumberString isEqualToString: @"0"]  ) {
                    [self addToLog: @"AxionVPN: Running the 'route-pre-down' script first."];
    ...
    [statusScreen setStatus: newState forName: [self displayName] connectedSince: [self timeString] to:remoteIP oftype:acctType];
    ...
    if (  [[configPath pathExtension] isEqualToString: @"tblk"]  ) {
        ...
        if (  status == EXIT_SUCCESS  ) {
            if ( [stdOutString hasPrefix: @"No such script exists: "]  ) {
                [self addToLog: [NSString stringWithFormat: @"AxionVPN: No '%@.sh' script to execute", scriptName]];
            } else {
                [self addToLog: [NSString stringWithFormat: @"AxionVPN: The '%@.sh' script executed successfully", scriptName]];
            }
        } else {
            ...
            [msg appendString: [NSString stringWithFormat: @"AxionVPN: The '%@.sh' script failed; 'openvpnstart %@' returned error %ld\n",
            ...
            if (   ( ! [scriptName isEqualToString: @"post-disconnect"])
                ...
                [self addToLog: [NSString stringWithFormat: @"AxionVPN: Disconnecting because the '%@.sh' script failed", scriptName]];
				[self startDisconnectingUserKnows: [NSNumber numberWithBool: YES]];
            }
        }
    }
