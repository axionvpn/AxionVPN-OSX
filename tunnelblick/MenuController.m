...
#import "AuthAgent.h"
#import "Axion/AxionAPI.h"
#import "Axion/AxionNetworkView.h"
#import "Axion/AxionNetworkSelect.h"
...
NSString              * gDeployPath = nil;            // Path to AxionVPN.app/Contents/Resources/Deploy
...
NSString *memUserName;          //User name cached in memory only
NSString *memPassword;          //Password cached in memory only
...
-(void)CheckAxionSite{
    ...
    NSString *axionURL = @"https://axionvpn.com";
    ...
    NSURLSessionDownloadTask *checkAxionTask =
    [session downloadTaskWithURL:[NSURL URLWithString:axionURL]
               ...
               completionHandler:^(NSURL *location,
                   ...
                   if (!error) {
                       ...
                       if (httpResp.statusCode == 200) {
                           NSLog(@"Connected");
                           gConnectsToSite = TRUE;
                           
                       }else{
                           NSLog(@"Response code: %@",httpResp.statusCode);
                       }
...
-(BOOL)InternetCheck
{
    ...
    if([self isNetworkAvailable] == FALSE){
        ...
        NSAlert *alert = [NSAlert alertWithMessageText:@"Could not resolve axionvpn.com"
    ...
    //Prime the global to be false
    gConnectsToSite = FALSE;
    ...
    //Now lets insure we can connect to Axion
    [self CheckAxionSite];
    ...
    //Wait on semaphone and setting global value
    if(gConnectsToSite == FALSE){
        NSLog(@"Can't connect to Axion's site");
        
        NSAlert *alert = [NSAlert alertWithMessageText:@"Could not connect to AxionVPN"
                                         defaultButton:@"OK" alternateButton:nil otherButton:nil
                             informativeTextWithFormat:@"Please make sure you're connected to the Internet"];
        ...
        NSLog(@"Successfully connected to Site");
    }
...
{	
    if (  (self = [super init])  ) {
        ...
        [self checkPlist: @"/Library/Preferences/com.AxionVPN.AxionVPN.plist" renameIfBad: NO];
        [self checkPlist: [NSHomeDirectory() stringByAppendingPathComponent: @"Library/Preferences/com.AxionVPN.AxionVPN.plist"] renameIfBad: YES];
		...
		// Remove any old "launch AxionVPN" link in the private configurations folder
		NSString * tbLinkPath = [gPrivatePath stringByAppendingPathComponent: @"launch AxionVPN"];
        ...
        if (  ! [gTbDefaults boolForKey: @"doNotShowSplashScreen"]  ) {
            ...
            NSString * text = NSLocalizedString(@"Starting AxionVPN...", @"Window text");
        ...
        //Clean old configs
        [[ConfigurationManager manager] CleanConfigurationsAxion];
...
#define NSWorkspaceActiveDisplayDidChangeNotification @"NSWorkspaceActiveDisplayDidChangeNotification"
    ...
    NSLog(@"Axion Code Starts");
    
    NSLog(@"Checking for Internet Connectivity");
        if([self InternetCheck] == NO){
        exit(3);
    }
    
    [((MenuController *)[NSApp delegate]) showAxionNetworksWasClicked: self];
...
-(BOOL) checkPlist: (NSString *) path renameIfBad: (BOOL) renameIfBad {
    ...
    if (  renameIfBad  ) {
        ...
        TBShowAlertWindow(NSLocalizedString(@"Warning", @"Window title"),
						 NSLocalizedString(@"The AxionVPN preferences were corrupted and have been cleared. (The old preferences were renamed.)\n\nSee the Console Log for details.", @"Window text"));
    }
...
- (void) createStatusItem {
    ...
    if (  [statusItem respondsToSelector: @selector(button)]  ) {
        ...
        [statusItemButton setImage:[NSImage imageNamed:@"disconnected"]];
...
-(void) createMenu 
{
   ...
   //[noConfigurationsItem release];
    ...
    [axionNetworksItem release];
    axionNetworksItem = [[NSMenuItem alloc] init];
    [axionNetworksItem setTitle: NSLocalizedString(@"Axion Networks", @"Menu item")];
    [axionNetworksItem setTarget: self];
    [axionNetworksItem setAction: @selector(showAxionNetworksWasClicked:)];
    [axionNetworksItem setEnabled:YES];
    
    
    [vpnDetailsItem release];
    vpnDetailsItem = [[NSMenuItem alloc] init];
    [vpnDetailsItem setTitle: NSLocalizedString(@"Settings", @"Menu item")];
    ...
    [quitItem setTitle: NSLocalizedString(@"Quit AxionVPN", @"Menu item")];
    ...
    [myVPNMenu addItem:axionNetworksItem];
        ...
        for (  i=theIndex; i < [theMenu numberOfItems]; i++  ) {
            ...
            if (   [menuItem submenu]  ) {    // item is a submenu
                menuItemTitle = [menuItem title];
					   ...
					   || [[menuItem title] isEqualToString: NSLocalizedString(@"Settings", @"Menu item")]  ) {
                break;
    ...
    if (  feedURL  ) {
        if (  [updater respondsToSelector: @selector(setFeedURL:)]  ) {
			...
			if (  [ext length] == 0  ) {
				...
				[self setFeedURL: [NSString stringWithFormat: @"%@.%@", withoutExt, ext]];
			}
                            ...
                            workingName: (NSString *) workingName
{
    ...
    NSLog(@"the name: %@ index: %d workingName %@",theName,theIndex,workingName);
...
- (BOOL)validateMenuItem:(NSMenuItem *)anItem 
{
    ...
    if (  act == @selector(disconnectAllMenuItemWasClicked:)  ) {
        ...
        if (  nConnections == 0  ) {
            ...
            myState = [NSString stringWithFormat: NSLocalizedString(@"Disconnect (%@)", @"Status message"), name];
            [statusMenuItem setTitle: myState];
        } else {
            myState = [NSString stringWithFormat:NSLocalizedString(@"Disconnect (%d Connections)", @"Status message"),nConnections];
            [statusMenuItem setTitle: myState];
        }
    } else {
...
-(void) addNewConfig: (NSString *) path withDisplayName: (NSString *) dispNm
{
    ...
    NSUInteger itemsToSkip = 4; //(  showVpnDetailsAtTop
							  //? 4
							  //: 2);
    ...
    NSLog(@"Config added to dictionary");
    
    //Axion - connect
    [self connectToConf:dispNm];
    
}

// Add new config to myVPNConnectionDictionary, the menu, and myConfigDictionary
// Note: The menu item's title will be set on demand in VPNConnection's validateMenuItem. We do this to
// Normal openvpn files without the tblk code
-(void) addNewConfigAxion: (NSString *) path withDisplayName: (NSString *) dispNm
{
    if (  invalidConfigurationName(dispNm, PROHIBITED_DISPLAY_NAME_CHARACTERS_CSTRING)  ) {
        TBShowAlertWindow(NSLocalizedString(@"Name not allowed", @"Window title"),
                          [NSString stringWithFormat: NSLocalizedString(@"Configuration '%@' will be ignored because its"
...
-(void) deleteExistingConfig: (NSString *) dispNm
{
    ...
    if (   myConnection
        ...
        [myConnection addToLog: @"*AxionVPN: Disconnecting; user asked to delete the configuration"];
        ...
        TBShowAlertWindow([NSString stringWithFormat: NSLocalizedString(@"'%@' has been disconnected", @"Window title"), localName],
						 [NSString stringWithFormat: NSLocalizedString(@"AxionVPN has disconnected '%@' because its configuration file has been removed.", @"Window text"), localName]);
    }
	...
	BOOL showVpnDetailsAtTop = (   ( ! [gTbDefaults boolForKey:@"doNotShowVpnDetailsMenuItem"] )
								&& ( ! [gTbDefaults boolForKey:@"putVpnDetailsAtBottom"] ) );
    int itemsToSkip = 4;// ( showVpnDetailsAtTop ? 4 : 2 );
...
- (NSString *) openVPNLogHeader
{
    ...
    return ([NSString stringWithFormat:@"AxionVPN: OS X %@; %@%@", currentVersionString, tunnelblickVersion([NSBundle mainBundle]), priorVersionString]);
}
...
- (void) checkForUpdates: (id) sender
{
    ...
    if (   [gTbDefaults boolForKey:@"onlyAdminCanUpdate"]
        ...
        if (  [updater respondsToSelector: @selector(checkForUpdates:)]  ) {
            if (  feedURL != nil  ) {
                if (  ! userIsAnAdmin  ) {
                    int response = TBRunAlertPanelExtended(NSLocalizedString(@"Only computer administrators should update AxionVPN", @"Window title"),
                                                           NSLocalizedString(@"You will not be able to use AxionVPN after updating unless you provide an administrator username and password.\n\nAre you sure you wish to check for updates?", @"Window text"),
        ...
        while (  (connection = [e nextObject])  ) {
            if (  ! [connection isDisconnected]  ) {
                ...
                if (   ( ! gShuttingDownWorkspace)
                    ...
                    && ( ! quittingTunnelblick)  ) {
                    [connection addToLog: [NSString stringWithFormat: @"AxionVPN: %@", logMessage]];
                }
...
-(void) waitForDisconnection: (NSMutableArray *) connectionsList {
	...
	if (  [connectionsList count] != 0  ) {
       ...
       // [((MenuController *)[NSApp delegate]) deleteExistingConfig:@"axion"];
...
-(void) doDisconnectionsForQuittingTunnelblick {
    ...
    NSMutableArray * connectionsToWaitFor = [self startDisconnecting: disconnectList
                                                 ...
                                                 quittingTunnelblick: YES
                                                          logMessage: @"Disconnecting because quitting AxionVPN"];
    [self waitForDisconnection: connectionsToWaitFor];
    
    status = pthread_mutex_unlock( &doDisconnectionsMutex );
    if (  status != EXIT_SUCCESS  ) {
        NSLog(@"doDisconnectionsForQuittingAxionVPN: pthread_mutex_unlock( &doDisconnectionsMutex ) failed; status = %ld, errno = %ld", (long) status, (long) errno);
    }
}
...
-(void) checkNoConfigurations {
    ...
    // If this is a Deployed version with no configurations, Quit AxionVPN
    if (   [gConfigDirs count] == 1
        ...
        TBRunAlertPanel(NSLocalizedString(@"All configuration files removed", @"Window title"),
                        NSLocalizedString(@"All configuration files have been removed. AxionVPN must quit.", @"Window text"),
                        nil, nil, nil);
        [self terminateBecause: terminatingBecauseOfError];
    }
    ...
    if (  anyNonTblkConfigs()  ) {
        ...
        int response = TBRunAlertPanel(NSLocalizedString(@"Warning", @"Window title"),
                                       NSLocalizedString(@"You have OpenVPN configurations but you do not have any AxionVPN VPN Configurations.\n\n"
                                                         @"The OpenVPN configurations must be converted to AxionVPN VPN Configurations if you want to use them.\n\n", @"Window text"),
...
-(IBAction) showAxionNetworksWasClicked: (id) sender
{
    (void) sender;
    
    //AXION SHOW Window
    if(self.AxionWindow == nil){
         NSLog(@"[showAxionNetworksWasClicked] self.AxionWindow nil, allocating");
        self.AxionWindow = [[AxionNetworkSelect alloc] initWithWindowNibName: @"AxionNetworkSelect"];
    }
    
    
    if(self.AxionNetworkWindow == nil){
        NSLog(@"nil Window, reloading");
        [self.AxionWindow showWindow:self.AxionWindow];
        self.AxionNetworkWindow = [self.AxionWindow window];
    }
    
    
    if(self.AxionNetworkWindow == nil){
         NSLog(@"MyWindow still nil");
    }
    
    [self.AxionNetworkWindow makeKeyAndOrderFront: nil];
...
-(IBAction) disconnectAllMenuItemWasClicked: (id) sender
{
    ...
    while (  (connection = [connEnum nextObject])  ) {
        if (  ! [connection isDisconnected]  ) {
            [connection addToLog: @"*AxionVPN: Disconnecting; 'Disconnect' menu command invoked"];
            [connection startDisconnectingUserKnows: [NSNumber numberWithBool: YES]];
        }
    }
}
    ...
    if (  ! [gTbDefaults preferenceExistsForKey: @"updateCheckAutomatically"]  ) {
        ...
        if (  obj  ) {
            if (  [obj respondsToSelector:@selector(boolValue)]) {
                ...
                NSLog(@"Preference 'SUEnableAutomaticChecks' is not a boolean and is being ignored by AxionVPN");
            }
        }
    }
    ...
    if (  ! [gTbDefaults preferenceExistsForKey: @"updateSendProfileInfo"]  ) {
        ...
        if (  obj  ) {
            if (  [obj respondsToSelector:@selector(boolValue)]  ) {
                ...
                NSLog(@"Preference 'SUSendProfileInfo' is not a boolean and is being ignored by AxionVPN");
            }
        }
    }
...
{
	// Invoked when the Dock item is clicked to relaunch AxionVPN, or it is double-clicked.
	// Just show the VPN Details… window.
	
	(void) theApp;
	(void) hasWindows;
	
	[self showAxionNetworksWasClicked: self];
	return NO;
}
...
- (void) applicationDidFinishLaunching: (NSNotification *)notification
{
	...
	// of AxionVPN.app that is on the disk image.
    if (   ( ! [gTbDefaults boolForKey: @"doNotEjectTunnelblickVolume"] )
        && [gFileMgr fileExistsAtPath: @"/Volumes/AxionVPN/AxionVPN.app"]  ) {
		NSString * outString = nil;
		NSString * errString = nil;
        NSArray * args = [NSArray arrayWithObjects: @"eject", @"/Volumes/AxionVPN", nil];
        OSStatus status = runTool(TOOL_PATH_FOR_DISKUTIL, args, &outString, &errString);
        if (  status != 0  ) {
            NSLog(@"diskutil eject /Volumes/AxionVPN failed with status %ld; stdout =\n%@\nstderr =\n%@", (long) status, outString, errString);
        }
    }
    ...
    NSString * text = NSLocalizedString(@"AxionVPN is ready.", @"Window text");
    [splashScreen setMessage: text];
...
-(void) initialChecks: (NSString *) ourAppName
{
    ...
    // If necessary, (re)install AxionVPN in /Applications
...
-(void) relaunchIfNecessary
{
    ...
    // Move or copy AxionVPN.app to /Applications if it isn't already there
    
    BOOL canRunOnThisVolume = [self canRunFromVolume: currentPath];
    
    if (  canRunOnThisVolume ) {
#ifdef TBDebug
        NSLog(@"Tunnelblick: WARNING: This is an insecure copy of AxionVPN to be used for debugging only!");
        return;
#endif
        if (  [currentPath isEqualToString: @"/Applications/AxionVPN.app"]  ) {
            return;
        } else {
            NSLog(@"AxionVPN can only run when it is /Applications/AxionVPN.app; path = %@.", currentPath);
        }
    } else {
        NSLog(@"AxionVPN cannot run when it is on /%@ because the volume has the MNT_NOSUID statfs flag set.", [[currentPath pathComponents] objectAtIndex: 1]);
    }
    ...
    if (  [gFileMgr fileExistsAtPath: tbInApplicationsPath]  ) {
        ...
        launchWindowText = NSLocalizedString(@"AxionVPN was successfully replaced.\n\nDo you wish to launch the new version of AxionVPN now?", @"Window text");
    } else {
        authorizationText = [NSString stringWithFormat:
                             NSLocalizedString(@"Do you wish to install %@ to %@%@?\n\n", @"Window text"),
                             appVersion, applicationsDisplayName, tblksMsg];
        launchWindowText = NSLocalizedString(@"AxionVPN was successfully installed.\n\nDo you wish to launch AxionVPN now?", @"Window text");
    }
    ...
    gAuthorization = [NSApplication getAuthorizationRef:
                      [[[NSLocalizedString(@" AxionVPN must be installed in Applications.\n\n", @"Window text")
						 stringByAppendingString: authorizationText]
                        stringByAppendingString: convertTblksText]
					   stringByAppendingString: @""]
					  ];
	if (  ! gAuthorization  ) {
		NSLog(@"The AxionVPN installation was cancelled by the user.");
		[self terminateBecause: terminatingBecauseOfQuit];
	}
    ...
    while (  numberOfOthers > 0  ) {
        int button = TBRunAlertPanel(NSLocalizedString(@"Tunnelblick is currently running", @"Window title"),
                                     ...
                                     NSLocalizedString(@"Cancel",  @"Button"),   // Alternate button
                                     nil);
        if (  button != NSAlertDefaultReturn  ) {// Cancel or error: Quit AxionVPN
            [self terminateBecause: terminatingBecauseOfQuit];
        }
    ...
    [splashScreen setMessage: NSLocalizedString(@"Installing and securing AxionVPN...", @"Window text")];
    ...
    if (  response == NSAlertDefaultReturn  ) {
        ...
        if (  ! [[NSWorkspace sharedWorkspace] launchApplication: tbInApplicationsPath]  ) {
            TBRunAlertPanel(NSLocalizedString(@"Unable to launch AxionVPN", @"Window title"),
...
-(void) secureIfNecessary
{
    ...
    if (  (installFlags = needToRunInstaller(FALSE)) != 0  ) {
        ...
        [splashScreen setMessage: NSLocalizedString(@"Securing AxionVPN...", @"Window text")];
		...
		installTblksFirst: (NSArray *) tblksToInstallFirst
{
    ...
    if (  *authRefPtr == NULL  ) {
        ...
        if (  message  ) {
            ...
            msg = [NSMutableString stringWithString: NSLocalizedString(@"AxionVPN needs to:\n", @"Window text")];
...
-(NSInteger) runInstallerAxion: (unsigned) installFlags
           extraArguments: (NSArray *) extraArguments
          usingAuthRefPtr: (AuthorizationRef *) authRefPtr
                  message: (NSString *) message
        installTblksFirst: (NSArray *) tblksToInstallFirst
{
    ...
    NSLog(@"[runInstallerAxion] Called");
...
BOOL needToChangeOwnershipAndOrPermissions(BOOL inApplications)
{
	// Check ownership and permissions on components of AxionVPN.app
	...
	if (  runningOnLeopardOrNewer()  ) {
		if (  [gFileMgr fileExistsAtPath: TUNNELBLICKD_PLIST_PATH]  ) {
			...
			NSLog(@"Need to install AxionVPNd plist at %@", TUNNELBLICKD_PLIST_PATH);
			return YES;
		}
	}
...
-(void)finishWakingUpFromSleep {
	...
	while (  (connection = [e nextObject])  ) {
        ...
        if (  ! [gTbDefaults boolForKey: key]  ) {
            ...
            [connection addToLog: @"AxionVPN: Woke up from sleep. Attempting to re-establish connection..."];
            ...
            [connection addToLog: @"AxionVPN: Woke up from sleep. Not attempting to re-establish connection..."];
        }
	}
...
-(void)didBecomeInactiveUser
{
	...
	while (  (connection = [e nextObject])  ) {
        if (   [connection shouldDisconnectWhenBecomeInactiveUser]
            ...
            [connection addToLog: @"AxionVPN: Disconnecting; user became inactive"];
            [connection startDisconnectingUserKnows: [NSNumber numberWithBool: YES]];
            ...
            [connection addToLog: @"AxionVPN: Stopping communication with OpenVPN because user became inactive"];
...
-(void)reconnectAfterBecomeActiveUser
{
	...
	if (  [[self connectionsToRestoreOnUserActive] count] == 0  ) {
		...
		while (  (connection = [e nextObject])  ) {
			if (  ! [connection isHookedup]  ) {
				[connection stopTryingToHookup];
				[connection addToLog: @"AxionVPN: Attempting to reconnect because user became active"];
				[connection connect: self userKnows: YES];
                ...
                forDisplayName: (NSString *) theName
{
	(void) ctl;
     ...
     NSLog(@"[statusWindowController] confDictionary: %@", [self myVPNConnectionDictionary]);
    ...
    if (  connection  ) {
        if (  choice == statusWindowControllerDisconnectChoice  ) {
            [connection addToLog: @"AxionVPN: Disconnecting; notification window disconnect button pressed"];
            ...
            [connection addToLog: @"AxionVPN: Connecting; notification window connect button pressed"];
            [connection connect: self userKnows: YES];
        } else {
...
//Axion to connect once we have a config
-(void) connectToConf:(NSString *)confName{
    NSLog(@"[connectToConf] Attempting to connect to %@",confName);
    NSLog(@"confDictionary: %@", [self myVPNConnectionDictionary]);
    VPNConnection * connection = [[self myVPNConnectionDictionary] objectForKey: confName];
    NSLog(@"[connectToConf] Connection object created");
   // [connection addToLog: @"*AxionVPN: Connecting; auto-connect after obtaining config"];
    if(connection){
      [connection connect: self userKnows: YES];
    }else{
        NSLog(@"[connectToConf] Failed to get connection for %@",confName);
    }
    NSLog(@"[connectToConf] Returning");
...
//Axion to connect once we have a config
-(void) connectToConfAxion:(NSString *)confName{
    NSLog(@"[connectToConf] Attempting to connect to %@",confName);
    NSLog(@"confDictionary: %@", [self myVPNConnectionDictionary]);
    VPNConnection * connection = [[self myVPNConnectionDictionary] objectForKey: confName];
    NSLog(@"[connectToConf] Connection object created");
    // [connection addToLog: @"*AxionVPN: Connecting; auto-connect after obtaining config"];
    if(connection){
        [connection connectNoChecks: self userKnows: YES];
    }else{
        NSLog(@"[connectToConf] Failed to get connection for %@",confName);
    }
    NSLog(@"[connectToConf] Returning");
...
-(NSImage *) largeMainImage
{
   ...
   //Jackson return [NSImage imageNamed:@"imageName.png/jpg and so on"];
}
