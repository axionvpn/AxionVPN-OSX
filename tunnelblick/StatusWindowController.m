...
-(void) awakeFromNib
{
    [self setStatus: status forName: name connectedSince: connectedSince to:remoteIP oftype:acctType];
    ...
    [self setSizeAndPosition];
    [[self window] setTitle: NSLocalizedString(@"AxionVPN", @"Window title")];
...
//Elmo, do stuff here
-(void) setStatus: (NSString *) theStatus forName: (NSString *) theName connectedSince: (NSString *) theTime to:(NSString * )axionIP oftype:(NSString *)accountType
{
    if (  gShuttingDownWorkspace  ) {
        return;
    }
    
    //Get the conf-info remotely
    ...
    if(axionIP != nil){
        [self setRemoteIP:axionIP];
    }
    
    
    
    if(accountType != nil){
        [self setAcctType:accountType];
        [configurationNameTFC setStringValue:[NSString stringWithFormat:@"%@ (%@)",localName,accountType]];
    }else{
        [configurationNameTFC setStringValue:[NSString stringWithFormat:@"%@",localName]];
    }
    ...
    if (   [theStatus isEqualToString: @"EXITING"]  ) {
        ...
        if(axionIP){
            [statusTFC            setStringValue: [NSString stringWithFormat: @"%@ %@ to %@",
                                               localizeNonLiteral(theStatus, @"Connection status"),
                                              [self connectedSince],axionIP]];
        }
...
TBSYNTHESIZE_OBJECT(retain, NSString *, remoteIP, setRemoteIP)
TBSYNTHESIZE_OBJECT(retain, NSString *,  acctType, setAcctType)
