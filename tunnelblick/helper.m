...
NSString * tunnelblickVersion(NSBundle * bundle)
{
    ...
    if (  [[infoVersion class] isSubclassOfClass: [NSString class]] && [infoVersion rangeOfString: @"3.0b"].location == NSNotFound  ) {
        ...
        return [NSString stringWithFormat: @"AxionVPN %@", infoShort];
    }
    ...
    NSString * appVersion = (  [infoShort hasPrefix: @"AxionVPN "]
                             ? [infoShort substringFromIndex: [@"AxionVPN " length]]
                             : infoShort);
    ...
    [version appendString: NSLocalizedString(@"AxionVPN", @"Window title")];
...
NSString * copyrightNotice()
{
    ...
    //return [NSString stringWithFormat:
      //      NSLocalizedString(@"Copyright © 2015-%@ Angelo Laub and others.", @"Window text"),
        //    year];
    return [NSString stringWithFormat:
            NSLocalizedString(@"Copyright © %@ Axion.", @"Window text"),
            year];
}
...
{
    NSLog(@"[helper:runOpenvpnstart] Called with %@",arguments);
    ...
    NSLog(@"[helper:runOpenvpnstart] Checkpoint 1");
    ...
    if (  runningOnLeopardOrNewer()  ) {
        NSLog(@"[helper:runOpenvpnstart] Checkpoint 2");
        ...
        NSLog(@"[helper:runOpenvpnstart] Checkpoint 3");
        ...
        if (  (perms & S_ISUID) == 0  ) {
            NSLog(@"runOpenvpnstart: This program has not been secured. launch AxionVPN to secure this program.");
            return -1;
        }
    ...
    if (  stdoutString  ) {
        *stdoutString = myStdoutString;
        ...
        if (  [myStdoutString length] != 0  ) {
            [logMsg appendFormat: @"AxionVPN stdout:\n'%@'\n", myStdoutString];
        }
    }
    NSLog(@"[helper:runOpenvpnstart] stdout: %@",myStdoutString);
    
    if (  stderrString  ) {
        *stderrString = myStderrString;
    } else {
        if (  [myStderrString length] != 0  ) {
            [logMsg appendFormat: @"AxionVPN stderr:'%@'\n", myStderrString];
        }
    }
