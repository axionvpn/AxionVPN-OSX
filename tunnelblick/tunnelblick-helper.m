...
NSString			* gResourcesPath = nil;     // Path to AxionVPN.app/Contents/Resources
...
void printUsageMessageAndExitOpenvpnstart(void) {
    ...
    fprintf(stderr,
            ...
            "                (The standard scripts are AxionVPN.app/Contents/Resources/client.up.tunnelblick.sh & client.down.tunnelblick.sh,\n"
            ...
            "           or 2 to use the Deploy folder in AxionVPN.app/Contents/Resources for configuration and other files,\n"
    ...
    exitOpenvpnstart(OPENVPNSTART_RETURN_SYNTAX_ERROR);      // This exit code is used in the VPNConnection connect: method to inhibit display of this long syntax error message because it means there is an Internal AxionVPN error
}
	...
	if (  ! [path isEqualToString: @"INVALID"]  ) {
		...
		if (  [path hasPrefix: @"/A"]  ) {
            ...
            if (   (   [[pathComponents objectAtIndex: 0] isEqualToString: @"/"]
                    ...
                    && [[pathComponents objectAtIndex: 2] isEqualToString: @"AxionVPN.app"]
...
NSString * createScriptLog(NSString* configurationFile, unsigned cfgLocCode) {
    ...
    NSString * dateCmdLine = [NSString stringWithFormat:@"%@ AxionVPN: openvpnstart starting OpenVPN\n",[date descriptionWithCalendarFormat:@"%a %b %e %H:%M:%S %Y"]];
...
int startVPN(NSString * configFile,
    ...
    switch (cfgLocCode) {
        ...
        case CFG_LOC_ALTERNATE:
           ...
           // cdFolderPath = [NSString stringWithFormat:@"%@/%@", L_AS_T_USERS, gUserName];  // Will be set below BECAUSE this is a .tblk.
            ...
            cdFolderPath = [NSString stringWithFormat:@"/Users/%@/Library/Application Support/Tunnelblick/Configurations", gUserName];  // Will be set below BECAUSE this is a .tblk.
            fprintf(stdout,"cdFolderPath: %s\n",[cdFolderPath UTF8String]);
            
            //NSString *cdFolderPath2 = [NSString stringWithFormat:@"/Users/%@/Library/Application Support/Tunnelblick/Configurations", gUserName];  // Will be set below BECAUSE this is a .tblk.
            
            //fprintf(stdout,"cdFolderPath2: %s\n",[cdFolderPath2 UTF8String]);

            
           // cdFolderPath = [NSString stringWithFormat:@"/Users/user/Library/Application Support/Tunnelblick/Configurations"];  // Will be set below BECAUSE this is a .tblk.
            
           // /Users/user/Library/Application Support/Tunnelblick/Configurations/axion.tblk/Contents/Resources

            gConfigPath  = [[cdFolderPath stringByAppendingPathComponent: configFile] copy];
            //NSString *gConfigPath2  = [[cdFolderPath stringByAppendingPathComponent: configFile] copy];
            
            fprintf(stdout,"gConfigPath: %s\n",[gConfigPath UTF8String]);
            //fprintf(stdout,"gConfigPath2: %s\n",[gConfigPath2 UTF8String]);
    ...
    if (  [[gConfigPath pathExtension] isEqualToString: @"tblk"]) {
        ...
        fprintf(stdout,"new gConfigPath: %s\n",[gConfigPath UTF8String]);
    ...
    if(  useScripts != 0  ) {  // 'Set nameserver' specified, so use our standard scripts or Deploy/<config>.up.sh and Deploy/<config>.down.sh
        ...
        if (   ([upscriptCommand length] > 199  )
            ...
            fprintf(stderr, "Warning: Path for up and/or down script is very long. OpenVPN truncates the command line that starts each script to 255 characters, which may cause problems. Examine the OpenVPN log in Tunnelblick's \"Settings\" window carefully.\n");
        }
...
int main(int argc, char * argv[]) {
    ...
    if (   ([execComponents count] != 5)
        ...
        //                                                  Allow any name for AxionVPN.app
